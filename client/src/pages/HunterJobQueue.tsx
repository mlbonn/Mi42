/**
 * FRIDAY CRM - Hunter Job Queue
 * Monitoring and management of Hunter Agent jobs
 */
...
export default function HunterJobQueue() {
  const queryClient = useQueryClient();
  const [selectedStatus, setSelectedStatus] = useState<string>("all");
  const [autoRefresh, setAutoRefresh] = useState(true);

  const { data: jobs = [], isLoading } = useQuery<HunterJob[]>({
    queryKey: ["/api/hunter/queue", selectedStatus],
    queryFn: async () => {
      const params = selectedStatus !== "all" ? `?status=${selectedStatus}` : "";
      const res = await fetch(`/api/hunter/queue${params}`, { credentials: "include" });
  ...
  const retryMutation = useMutation({
    mutationFn: async (jobId: string) => {
      const res = await fetch(`/api/hunter/queue/${jobId}/retry`, {
        method: "POST",
        credentials: "include",
      });
      if (!res.ok) throw new Error(await res.text());
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/hunter/queue"] }),
  });
  ...
  const getDuration = (job: HunterJob) => {
  ...
  return (
    <div className="container py-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Hunter Job Queue</h1>
          <p className="text-sm text-gray-600">Monitor and manage Hunter Agent jobs</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/hunter/queue"] })}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    ...
                    <th className="pb-2 font-medium text-gray-600">Corporation</th>
                    <th className="pb-2 font-medium text-gray-600">Status</th>
                    <th className="pb-2 font-medium text-gray-600">Contacts Found</th>
                ...
                <tbody>
                  {filteredJobs.map(job => (
                    <tr key={job.id} className="border-b hover:bg-gray-50">
                      <td className="py-2 font-mono text-xs text-gray-500">{job.id.slice(0, 8)}...</td>
                      <td className="py-2 font-mono text-xs">{job.corporationId?.slice(0, 12)}...</td>
                      <td className="py-2">{getStatusBadge(job.status)}</td>
                      <td className="py-2 text-center">{job.contactsFound ?? "-"}</td>
                      ...
                      <td className="py-2">
                        ...
                        {job.errorMessage && (
                          <span className="ml-2 text-xs text-gray-500" title={job.errorMessage}>
                            ⚠
                          </span>
                        )}
                      </td>
