...
export async function processHunterJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    // Load Hunter Settings
    const { getHunterSettings } = await import("../hunterDb");
    const settings = await getHunterSettings();
    
    console.log(`[HunterService] Using settings:`, {
      personTitles: settings?.personTitles?.length || 0,
    ...
    const targetRoles = (job.targetRoles as string[]) || settings?.personTitles || [];
    const targetSeniorities = settings?.personSeniorities || ["c_suite", "vp", "director"];
    const targetCount = settings?.targetCount || job.targetCount || 5;
    
    const apolloResults = await searchPeople({
      organizationDomain: domain,
      personTitles: targetRoles.length > 0 ? targetRoles : undefined,
      personSeniorities: targetSeniorities,
      perPage: targetCount
    });
