...
// Extract ALL TO addresses (not just first one)
const toAddresses = extractEmailAddresses(email.to);
const toAddressString = formatEmailAddresses(toAddresses);

// Extract ALL CC addresses (NEW!)
const ccAddresses = extractEmailAddresses(email.cc);
const ccAddressString = formatEmailAddresses(ccAddresses);
